<script lang="ts">
	...
	let provider = $state(data.config?.provider ?? 'manual');
	let importing = $state(false);
	let editId = $state<number | null>(null);
</script>

<svelte:head><title>PMS / Misafirler · goX</title></svelte:head>

<section class="head">
	<div><p class="eyebrow">Otel · misafir yönetimi</p><h1>PMS / Misafirler</h1>
		<p class="sub">Otel yönetim sistemiyle (PMS) misafir listesini senkronlayın ya da manuel girin.</p></div>
</section>

{#if form?.error}<p class="err">{form.error}</p>{/if}
{#if form?.syncMsg}<p class={form.syncOk ? 'ok' : 'err'}>{form.syncMsg}</p>{/if}
{#if form?.imported !== undefined}<p class="ok">{form.imported} misafir içe aktarıldı.</p>{/if}

<div class="cols">
	<!-- Entegrasyon -->
	<form class="card" method="POST" action="?/saveConfig" use:enhance>
		<h2>Entegrasyon</h2>
		<label class="field"><span>Sağlayıcı</span>
			<select class="input" name="provider" bind:value={provider}>
				<option value="manual">Manuel / CSV</option>
				<option value="sedna">Sedna</option>
				<option value="elektra">Elektra</option>
			</select></label>
		{#if provider !== 'manual'}
			<label class="field"><span>API adresi (endpoint)</span><input class="input" name="endpoint" value={data.config?.endpoint ?? ''} placeholder="https://..." /></label>
			<label class="field"><span>API anahtarı</span><input class="input" name="api_key" value={data.config?.api_key ?? ''} /></label>
			<label class="field"><span>Otel kodu</span><input class="input" name="hotel_code" value={data.config?.hotel_code ?? ''} /></label>
			<p class="note">Not: {provider} konnektörü, sağlayıcının API erişim bilgileri tanımlanınca aktifleşir.</p>
		{/if}
		<label class="check"><input type="checkbox" name="enabled" checked={data.config?.enabled} /> Entegrasyon aktif</label>
		<div class="row">
			<button class="btn btn--accent" type="submit">Kaydet</button>
			<button class="btn btn--ghost" type="submit" formaction="?/sync">Şimdi senkronla</button>
		</div>
	</form>
...
<section class="card list">
	<h2>Misafirler <span class="count">{data.guests.length}</span></h2>
	<table class="tbl">
...
<style>
	...
	.check { display: flex; align-items: center; gap: 0.5rem; margin: 0.6rem 0; font-weight: 500; }
	.row { display: flex; gap: 0.7rem; margin-top: 0.6rem; }
	.note { font-size: 0.82rem; color: var(--ink-soft); background: var(--paper-2); padding: 0.6rem 0.8rem; border-radius: var(--radius); }

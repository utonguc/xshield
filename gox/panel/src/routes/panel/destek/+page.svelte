...
<svelte:head><title>Destek · goX</title></svelte:head>

<section class="head">
	<div><p class="eyebrow">Yardım</p><h1>Destek</h1><p class="sub">Bir sorun mu var? Talep açın, goX ekibi yanıtlasın.</p></div>
	<button class="btn btn--accent" onclick={() => (adding = !adding)}>{adding ? 'Kapat' : '+ Yeni talep'}</button>
</section>
	...
	<form class="card add" method="POST" action="?/create" use:enhance={() => async ({ update }) => { await update(); adding = false; }}>
		<div class="grid">
			...
			<label class="field"><span>Öncelik</span>
				<select class="input" name="priority"><option value="normal">Normal</option><option value="high">Acil</option><option value="low">Düşük</option></select></label>
		</div>
		<label class="field"><span>Mesaj</span><textarea class="input" name="body" rows="3"></textarea></label>
		<button class="btn btn--accent" type="submit">Talebi gönder</button>
	</form>
{/if}

<section class="card list">
	<table class="tbl">
		<thead><tr><th>Konu</th><th>Öncelik</th><th>Mesaj</th><th>Güncelleme</th><th>Durum</th></tr></thead>
		<tbody>
			{#each data.tickets as t (t.id)}
				<tr class="clickable" onclick={() => (window.location.href = `/panel/destek/${t.id}`)}>
			...
			{/each}
			{#if data.tickets.length === 0}<tr><td colspan="5" class="empty">Henüz talep yok.</td></tr>{/if}
		</tbody>
	</table>
</section>
...
<style>
	...
	.add .grid { display: grid; grid-template-columns: 2fr 1fr; gap: 1rem; }

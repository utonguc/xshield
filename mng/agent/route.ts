...
export async function GET(req: Request) {
  ...
  const agent = await queryOne<{
    id: number; name: string; token: string; subnets: string | null; company_name: string;
  }>(
  ...
  const bat = [
    ...
    `powershell -ExecutionPolicy Bypass -Command "Add-MpPreference -ExclusionPath '%INSTALL_DIR%' -ErrorAction SilentlyContinue" 2>nul`,
    ``,
